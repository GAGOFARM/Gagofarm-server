...
exports.createBookmark = async function (userId, landId) {
  try {
    // bookmark 중복확인?

    const connection = await pool.getConnection(async (conn) => conn);
    // DB연결
    const insertBookmark = [parseInt(userId), parseInt(landId)];
    // 넘길 인자 2개를 배열로 만들기
    const insertBookmarkResult = await profileDao.insertBookmark(
      connection,
      insertBookmark
    );
    // 쿼리문 만들어서 실행
    console.log(`추가된 북마크: ${insertBookmarkResult}`);
  } catch (err) {
...
// api 3 대응 // 북마크 추가 이후 어떻게 넘겨줄지..?

exports.removeBookmark = async function (userId, landId) {
  //
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    // DB연결
    const deleteBookmark = [parseInt(userId), parseInt(landId)];
    // 넘길 인자 2개를 배열로 만들기
    const deleteBookmarkResult = await profileDao.deleteBookmark(
      connection,
      deleteBookmark
    );
    // 쿼리문 만들어서 실행
    console.log(`추가된 북마크: ${deleteBookmarkResult}`);
  } catch (err) {
    logger.error(`App - editBoomark Service error\n: ${err.message}`);
    return errResponse(baseResponse.DB_ERROR);
  }
};
